...
- (void)setupLayout {
    ...
    CGSize size = [self.messageImageview.image nyx_aspectFitSizeWithSize:CGSizeMake(200, 200)];
...
- (void)setMessage:(IMTopicMessage *)message {
    if (self.message && [self.message.uniqueID isEqualToString:message.uniqueID]) {
        //更新图片
        [self updateSendingStateWithMessage:message];
        WEAK_SELF
...
- (void)updateSendingStateWithMessage:(IMTopicMessage *)message {
